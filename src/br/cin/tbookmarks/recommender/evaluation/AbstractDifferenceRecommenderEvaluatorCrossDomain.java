 ...
 */
public abstract class AbstractDifferenceRecommenderEvaluatorCrossDomain implements RecommenderEvaluator {
	...
	/*private void removeTestsFromRescorer(FastByIDMap<PreferenceArray> testPrefs) {
		for (Map.Entry<Long,PreferenceArray> entry : testPrefs.entrySet()) {
			  long ids[] = entry.getValue().getIDs();
			  			
			  HashMap<Long,Float> itemValue = new HashMap<Long, Float>();
			  
		      for(int i = 0; i < ids.length; i++){
		    	  if(!this.idrescorer.isFiltered(ids[i])){
		    		  itemValue.put(ids[i], entry.getValue().getValue(i));
		    	  }
		      }
		      PreferenceArray value = new GenericUserPreferenceArray(itemValue.keySet().size());
		      value.setUserID(0, entry.getValue().getUserID(0));
		      
		      int index = 0;
		      
		      for (Long itemId : itemValue.keySet()) {
		    	  value.setItemID(index, itemId);
